...
const NOTE_SECTIONS = [
  ...
  },
  {
    ...
    icon: '✈️',
    title: '항공·이동',
    autoGenerated: true,   // HTML 렌더링 (공항 이동 루트 표)
  },
  {
    id: 'hotel',
    icon: '🏨',
...
function renderItin() {
  ...
  // 이름 클릭 → 인라인 편집 (startRename 공유)
  wrap.querySelectorAll('.editable-name').forEach(span => {
    span.addEventListener('click', e => {
      e.stopPropagation();
      const id = span.dataset.id;
      const dayNum = parseInt(span.dataset.day);
      startRename(id, dayNum, span);
    });
  });
...
const AIRPORT_LEGS = {
  1: [{ // Day 1: IST 도착
    pos: 'top',
    from: '✈️ IST 이스탄불 공항', to: '🕌 엘리시움 탁심호텔',
    icon: '🚌', mode: '하바이스트 HAV-4 (추천)',
    tip: '공항 1층 D2 출구 → 정류장 / 하차: 탁심광장 Point Hotel 앞',
    km: '약 45km · 80~100분 · 275 TL (약 9,500원)',
  }],
  5: [
    { // Day 5 오전: 탁심 → IST 출발
      pos: 'top',
      from: '🕌 엘리시움 탁심호텔', to: '✈️ IST 이스탄불 공항',
      icon: '🚌', mode: '하바이스트 HAV-4 (추천)',
      tip: '탁심광장 Point Hotel 앞 정류장 · 출발 10:00~10:30 권장',
      km: '약 45km · 80~100분 · 275 TL (약 9,500원)',
    },
    { // Day 5 오후: NAV 도착
      pos: 'bottom',
      from: '✈️ NAV 네브셰히르 공항', to: '🎈 로드오브카파도키아',
      icon: '🏨', mode: '호텔 픽업 (사전 예약 필수)',
      tip: '도착 터미널 출구 · 호텔 기사 이름 팻말 대기 · TK2006 15:05 도착',
      km: '약 15~20km · ~30분',
    },
  ],
  7: [
    { // Day 7 저녁: 카파도키아 → ASR 출발
      pos: 'top',
      from: '🎈 로드오브카파도키아', to: '✈️ ASR 카이세리 공항',
      icon: '🚗', mode: '호텔 샌딩 또는 사설 트랜스퍼',
      tip: '호텔 정문 픽업 · 출발 19:00~19:30 권장 · XQ7033 21:20 출발',
      km: '약 80km · 1~1.5시간 · 1,500~2,000 TL (약 51,000~68,000원)',
    },
    { // Day 7 밤: AYT 도착
      pos: 'bottom',
      from: '✈️ AYT 안탈리아 공항', to: '🏖️ 메가사라이 웨스트비치',
      icon: '🏨', mode: '호텔 픽업 (사전 예약 필수)',
      tip: '도착 터미널 출구 · XQ7033 22:40 도착 · 야간 도착',
      km: '약 25km · 22~32분 · 택시 $16~20 (약 22,000~28,000원)',
    },
  ],
};

function airportLegHtml(leg) {
  return `
    <div class="tg-leg tg-airport-leg">
      <div class="tg-leg-from tg-airport-from">${leg.from}</div>
      <div class="tg-leg-arrow">
        <span class="tg-leg-icon">${leg.icon}</span>
        <div class="tg-leg-info">
          <span class="tg-leg-mode">${leg.mode}</span>
          <span class="tg-leg-tip">${leg.tip}</span>
        </div>
        <span class="tg-leg-km">${leg.km}</span>
      </div>
      <div class="tg-leg-to tg-airport-to">${leg.to}</div>
    </div>`;
}
...
function buildTransportGuide() {
  ...
  for (let day = 1; day <= TOTAL_DAYS; day++) {
    const places  = state.itin[day] || [];
    const legs    = AIRPORT_LEGS[day] || [];
    const hasLegs = legs.length > 0;
    // 장소가 1개 이하이고 공항 레그도 없으면 skip
    if (places.length < 2 && !hasLegs) continue;
    ...
    if (cityTip) html += `<div class="tg-city-tip">💡 ${cityTip}</div>`;

    // 상단 공항 레그 (공항 출발/도착이 하루 시작)
    legs.filter(l => l.pos === 'top').forEach(l => { html += airportLegHtml(l); });
    ...
    // 하단 공항 레그 (하루 끝에 공항 이동)
    legs.filter(l => l.pos === 'bottom').forEach(l => { html += airportLegHtml(l); });
...
// 항공·이동 탭: 5개 루트 표 HTML
function buildFlightGuide() {
  return `
    <div class="fg-wrap">
      <div class="fg-title">✈️ 공항·호텔 이동 루트 <span class="fg-sub">1 TRY ≈ 34원 기준</span></div>
      ${[
        { day:'Day 1 입국', color:'#E57373', title:'IST 이스탄불 공항 → 엘리시움 탁심호텔',
          rows:[
            { rec:true,  icon:'🚌', label:'하바이스트 HAV-4', price:'275 TL · 약 9,500원', time:'80~100분', where:'공항 1층 D2 출구 → 정류장\n하차: 탁심광장 Point Hotel 앞' },
            { rec:false, icon:'🚇', label:'메트로 M11→M2',   price:'70 TL · 약 2,400원',   time:'45~55분',  where:'공항 지하 M11역\nGayrettepe 환승 → 탁심역' },
            { rec:false, icon:'🚕', label:'택시',            price:'1,200~2,000 TL · 약 41,000~68,000원', time:'45~60분', where:'입국장 바로 앞 택시 승강장' },
          ]},
        { day:'Day 5 출국', color:'#7986CB', title:'엘리시움 탁심호텔 → IST 이스탄불 공항',
          note:'TK2006 13:45 출발 · 호텔 출발 10:00~10:30',
          rows:[
            { rec:true,  icon:'🚌', label:'하바이스트 HAV-4', price:'275 TL · 약 9,500원', time:'80~100분', where:'탁심광장 Point Hotel 앞 정류장 (도보 5분)' },
            { rec:false, icon:'🚇', label:'메트로 M2→M11',   price:'70 TL · 약 2,400원',  time:'45~55분',  where:'탁심역 → Gayrettepe 환승 → 공항역' },
          ]},
        { day:'Day 5 도착', color:'#66BB6A', title:'NAV 네브셰히르 공항 → 로드오브카파도키아',
          note:'TK2006 15:05 도착 · 사전 예약 필수',
          rows:[
            { rec:true,  icon:'🏨', label:'호텔 픽업',  price:'호텔 문의', time:'~30분',    where:'도착 터미널 출구\n호텔 기사 이름 팻말 대기' },
            { rec:false, icon:'🚕', label:'택시',       price:'300~500 TL · 약 10,200~17,000원', time:'30~40분', where:'도착 터미널 정문 앞 택시 승강장' },
          ]},
        { day:'Day 7 출발', color:'#26A69A', title:'로드오브카파도키아 → ASR 카이세리 공항',
          note:'XQ7033 21:20 출발 · 호텔 출발 19:00~19:30 권장',
          rows:[
            { rec:true,  icon:'🏨', label:'호텔 샌딩',   price:'1,500~2,000 TL · 약 51,000~68,000원', time:'1~1.5시간', where:'호텔 정문 픽업' },
            { rec:false, icon:'🚌', label:'공유 셔틀',   price:'10~15 EUR · 약 15,000~22,000원',       time:'1.5시간+',  where:'마이리얼트립 등 사전 예약' },
          ]},
        { day:'Day 7 도착', color:'#42A5F5', title:'AYT 안탈리아 공항 → 메가사라이 웨스트비치',
          note:'XQ7033 22:40 도착 · 야간 도착',
          rows:[
            { rec:true,  icon:'🏨', label:'호텔 픽업', price:'호텔 문의', time:'~30분',   where:'도착 터미널 출구 · 호텔 기사 대기' },
            { rec:false, icon:'🚕', label:'택시',      price:'$16~20 · 약 22,000~28,000원', time:'22~32분', where:'도착 터미널 1층 출구 바로 앞 택시 승강장' },
          ]},
      ].map(r => `
        <div class="fg-route">
          <div class="fg-route-header">
            <span class="fg-badge" style="background:${r.color}">${r.day}</span>
            <span class="fg-route-title">${r.title}</span>
          </div>
          ${r.note ? `<div class="fg-note">${r.note}</div>` : ''}
          <div class="fg-rows">
            ${r.rows.map(o => `
              <div class="fg-row${o.rec ? ' fg-row-rec' : ''}">
                <span class="fg-icon">${o.icon}</span>
                <span class="fg-label">${o.label}${o.rec ? ' <span class="fg-rec">추천</span>' : ''}</span>
                <span class="fg-price">${o.price}</span>
                <span class="fg-time">${o.time}</span>
                <span class="fg-where">${o.where.replace(/\n/g,'<br>')}</span>
              </div>`).join('')}
          </div>
        </div>`).join('')}
    </div>`;
...
function initNotes() {
  ...
  function renderNotesPanel() {
    ...
    if (sec.autoGenerated) {
      ...
      guideDiv.innerHTML      = sec.id === 'flight' ? buildFlightGuide() : buildTransportGuide();
    } else {
